...
typedef struct __mavlink_ada_tm_t {
 ...
 float dpl_altitude; /*< [m] Main parachute deployment altitude*/
 uint32_t shadow_mode_time; /*< [ms] Shadow mode duration*/
 uint32_t apogee_timeout; /*< [ms] Apogee timeout*/
 uint8_t state; /*<  ADA current state*/
} mavlink_ada_tm_t;

#define MAVLINK_MSG_ID_ADA_TM_LEN 61
#define MAVLINK_MSG_ID_ADA_TM_MIN_LEN 61
#define MAVLINK_MSG_ID_205_LEN 61
#define MAVLINK_MSG_ID_205_MIN_LEN 61

#define MAVLINK_MSG_ID_ADA_TM_CRC 193
#define MAVLINK_MSG_ID_205_CRC 193
...
#define MAVLINK_MESSAGE_INFO_ADA_TM { \
    ...
    {  { "timestamp", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_ada_tm_t, timestamp) }, \
         { "state", NULL, MAVLINK_TYPE_UINT8_T, 0, 60, offsetof(mavlink_ada_tm_t, state) }, \
         ...
         { "shadow_mode_time", NULL, MAVLINK_TYPE_UINT32_T, 0, 52, offsetof(mavlink_ada_tm_t, shadow_mode_time) }, \
         { "apogee_timeout", NULL, MAVLINK_TYPE_UINT32_T, 0, 56, offsetof(mavlink_ada_tm_t, apogee_timeout) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_ADA_TM { \
 ...
 * @param dpl_altitude [m] Main parachute deployment altitude
 * @param shadow_mode_time [ms] Shadow mode duration
...
static inline uint16_t mavlink_msg_ada_tm_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint64_t timestamp, uint8_t state, float kalman_x0, float kalman_x1, float kalman_x2, float vertical_speed, float msl_altitude, float ref_pressure, float ref_altitude, float ref_temperature, float msl_pressure, float msl_temperature, float dpl_altitude, uint32_t shadow_mode_time, uint32_t apogee_timeout)
{
    ...
    _mav_put_uint32_t(buf, 52, shadow_mode_time);
    _mav_put_uint32_t(buf, 56, apogee_timeout);
...
static inline void mavlink_msg_ada_tm_send(mavlink_channel_t chan, uint64_t timestamp, uint8_t state, float kalman_x0, float kalman_x1, float kalman_x2, float vertical_speed, float msl_altitude, float ref_pressure, float ref_altitude, float ref_temperature, float msl_pressure, float msl_temperature, float dpl_altitude, uint32_t shadow_mode_time, uint32_t apogee_timeout)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ADA_TM_LEN];
    _mav_put_uint64_t(buf, 0, timestamp);
 ...
 */
static inline void mavlink_msg_ada_tm_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t timestamp, uint8_t state, float kalman_x0, float kalman_x1, float kalman_x2, float vertical_speed, float msl_altitude, float ref_pressure, float ref_altitude, float ref_temperature, float msl_pressure, float msl_temperature, float dpl_altitude, uint32_t shadow_mode_time, uint32_t apogee_timeout)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 *
 * @return [m] Main parachute deployment altitude
 */
...
/**
 * @brief Get field shadow_mode_time from ada_tm message
 *
 * @return [ms] Shadow mode duration
 */
static inline uint32_t mavlink_msg_ada_tm_get_shadow_mode_time(const mavlink_message_t* msg)
{
    return _MAV_RETURN_uint32_t(msg,  52);
}

/**
 * @brief Get field apogee_timeout from ada_tm message
 *
 * @return [ms] Apogee timeout
 */
static inline uint32_t mavlink_msg_ada_tm_get_apogee_timeout(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_ada_tm_decode(const mavlink_message_t* msg, mavlink_ada_tm_t* ada_tm)
{
    ...
    ada_tm->shadow_mode_time = mavlink_msg_ada_tm_get_shadow_mode_time(msg);
    ada_tm->apogee_timeout = mavlink_msg_ada_tm_get_apogee_timeout(msg);
