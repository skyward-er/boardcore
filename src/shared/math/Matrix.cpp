...
void Mat4::setInverse(const Mat4 &m) {
    ...
    d[3] = (m.d[9] * m.d[6] * m.d[3]
           -m.d[5] * m.d[10]* m.d[3]
           -m.d[9] * m.d[2] * m.d[7]
           +m.d[1] * m.d[10]* m.d[7]
           +m.d[5] * m.d[2] * m.d[11]
           -m.d[1] * m.d[6] * m.d[11])*det;

    d[7] = (-m.d[8] * m.d[6] * m.d[3]
            +m.d[4] * m.d[10]* m.d[3]
            +m.d[8] * m.d[2] * m.d[7]
            -m.d[0] * m.d[10]* m.d[7]
            -m.d[4] * m.d[2] * m.d[11]
            +m.d[0] * m.d[6] * m.d[11])*det;

    d[11] =(m.d[8] * m.d[5] * m.d[3]
           -m.d[4] * m.d[9] * m.d[3]
           -m.d[8] * m.d[1] * m.d[7]
           +m.d[0] * m.d[9] * m.d[7]
           +m.d[4] * m.d[1] * m.d[11]
           -m.d[0] * m.d[5] * m.d[11])*det;
}
...
Vec3 Mat3::operator*(const Vec3 &vector) const {
    return Vec3(
        vector.x * d[0] + vector.y * d[1] + vector.z * d[2],
        vector.x * d[3] + vector.y * d[4] + vector.z * d[5],
        vector.x * d[6] + vector.y * d[7] + vector.z * d[8]
    );    
}

Vec3 Mat3::transform(const Vec3 &vec) const {
    return (*this) * vec;
}
...
void Mat3::setInverse(const Mat3 &m)
{
    ...
    float t16 = (t4*m.d[8] - t6*m.d[7] - t8*m.d[8]+
                t10*m.d[7] + t12*m.d[5] - t14*m.d[4]);
