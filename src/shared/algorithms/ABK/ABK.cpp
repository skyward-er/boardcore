...
void ABK::step()
{
  ...
  float rtb_MatrixMultiply;
  float rtb_MatrixMultiply1;
  float rtb_Prelookup1_o2;
  ...
  // Outputs for Atomic SubSystem: '<Root>/ABK FICA - Autocoding'
  // If: '<S1>/If'
  ...
  u0 = ABK_P.Gain_Gain_l * ABK_U.ABKIn_e.ANASPosition[2];

  // Saturate: '<S23>/Limit  altitude  to troposhere'
  ...
  //   Constant: '<S23>/Sea Level  Temperature'
  ...
  if (rtAction == 0) {
    ...
    for (u0_tmp = 0; u0_tmp < 5; u0_tmp++) {
      ...
      rtb_MatrixMultiply = ABK_DW.Selecttrajectories[301 * u0_tmp + static_cast<
        int32_t>(rtb_Prelookup_o1)];
      rtb_Add1[u0_tmp] = (ABK_DW.Selecttrajectories[301 * u0_tmp +
                          rtb_Prelookup_o1_0] - rtb_MatrixMultiply) *
        rtb_Prelookup_o2 + rtb_MatrixMultiply;
    }
    ...
    rtb_OR_l = (rtb_Prelookup_o2 >= ABK_P.Bypass_vect_Bias[1] + rtb_Add1[4]);

    // If: '<S2>/If' incorporates:
    //   Constant: '<S18>/fullOpen'
    //   Constant: '<S7>/Full close'
    //   Constant: '<S8>/Full open'

    rtPrevAction = ABK_DW.If_ActiveSubsystem_e;
    ...
    ABK_DW.If_ActiveSubsystem_e = rtAction;
    switch (rtAction) {
     case 0:
     ...
     default:
      if (rtAction != rtPrevAction) {
        ...
        ABK_DW.Memory_PreviousInput_d = ABK_P.Memory_InitialCondition;

        // End of SystemReset for SubSystem: '<S5>/PID Controller'

        // SystemReset for Atomic SubSystem: '<S5>/Variant Filter'
        ABK_DW.Memory_PreviousInput[0] = ABK_P.Memory_InitialCondition_j;
        ABK_DW.Memory3_PreviousInput[0] = ABK_P.Memory3_InitialCondition;
        ABK_DW.Memory_PreviousInput[1] = ABK_P.Memory_InitialCondition_j;
        ABK_DW.Memory3_PreviousInput[1] = ABK_P.Memory3_InitialCondition;
        ABK_DW.Memory_PreviousInput_g = ABK_P.Memory_InitialCondition_b;
      ...
      rtb_Prelookup_o2 += (ABK_P.Extensions_Value[static_cast<uint16_t>
                           (rtb_Prelookup1_o1 + ABK_P.Bias_Bias_c)] -
                           rtb_Prelookup_o2) * rtb_MatrixMultiply;

      // Outputs for Atomic SubSystem: '<S5>/PID Controller'
      // Bias: '<S13>/refBias1'
      rtb_MatrixMultiply = rtb_Prelookup_o2 + ABK_P.refBias1_Bias;
      ...
      if (ABK_DW.Memory1_PreviousInput) {
        rtb_Prelookup1_o2 = ABK_P.Constant_Value;
      } else {
        rtb_Prelookup1_o2 = ABK_P.SampleTime_Value * rtb_MatrixMultiply;
      }
      ...
      rtb_MatrixMultiply1 = rtb_Prelookup1_o2 + ABK_DW.Memory_PreviousInput_d;
      ...
      rtb_MatrixMultiply = (((ABK_DW.UnitDelay_DSTATE + ABK_P.refBias_Bias) /
        ABK_P.SampleTime_Value * ABK_P.Kd_Value + ABK_P.Ki_Value *
        rtb_MatrixMultiply1) + ABK_P.Kp_Value * rtb_MatrixMultiply) +
        ABK_P.refBias2_Bias;

      // Saturate: '<S13>/Saturation'
      if (rtb_MatrixMultiply > ABK_P.Saturation_UpperSat) {
        rtb_MatrixMultiply = ABK_P.Saturation_UpperSat;
      } else if (rtb_MatrixMultiply < ABK_P.Saturation_LowerSat) {
        rtb_MatrixMultiply = ABK_P.Saturation_LowerSat;
      }
      ...
      ABK_DW.Memory1_PreviousInput = ((rtb_MatrixMultiply <=
        ABK_P.FullClose_compare_const) || (rtb_MatrixMultiply >=
        ABK_P.FullOpen_compare_const));

      // Update for Memory: '<S13>/Memory'
      ABK_DW.Memory_PreviousInput_d = rtb_MatrixMultiply1;
      ...
      rtb_MatrixMultiply1 = ABK_DW.Memory_PreviousInput[1];
      ...
      rtb_MatrixMultiply1 = ((ABK_P.coeffb_Value[0] * rtb_MatrixMultiply +
        ABK_DW.Memory_PreviousInput[0] * ABK_P.coeffb_Value[1]) +
        ABK_P.coeffb_Value[2] * rtb_MatrixMultiply1) - (ABK_P.coeffa_Value[0] *
        ABK_DW.Memory3_PreviousInput[0] + ABK_P.coeffa_Value[1] *
        ABK_DW.Memory3_PreviousInput[1]);
      ...
      rtb_OR = ((rtb_zNAS >= ABK_P.FullOpen_compare_const_o) ||
                ABK_DW.Memory_PreviousInput_g);

      // Outputs for Enabled SubSystem: '<S16>/Final full opening' incorporates:
      //   EnablePort: '<S18>/Enable'

      if (rtb_OR) {
        ABK_DW.fullOpen = ABK_P.fullOpen_Value;

        // Switch: '<S16>/Switch' incorporates:
        //   Constant: '<S18>/fullOpen'

        rtb_zNAS = ABK_DW.fullOpen;
      } else {
        // PreLookup: '<S20>/Prelookup1' incorporates:
        //   Constant: '<S20>/zColumn'
        //   Switch: '<S16>/Switch'

        rtb_Prelookup1_o1 = plook_u16fdf_evenx(rtb_zNAS, ABK_P.zColumn_Value[0],
          ABK_P.zColumn_Value[1] - ABK_P.zColumn_Value[0], 300U,
          &rtb_Prelookup1_o2);

        // Selector: '<S20>/Select on mass' incorporates:
        //   Constant: '<S20>/Trajectory difference'
        //   Selector: '<S20>/Selector'
        //   Selector: '<S20>/Selector1'
        //   Switch: '<S16>/Switch'

        u0_tmp = 301 * ABK_DW.massIndex;
        rtb_zNAS = ABK_P.Trajectorydifference_Value[u0_tmp + rtb_Prelookup1_o1];

        // Gain: '<S20>/T_refGain' incorporates:
        //   Bias: '<S20>/Bias'
        //   Constant: '<S20>/Trajectory difference'
        //   Product: '<S20>/Product'
        //   Selector: '<S20>/Select on mass'
        //   Selector: '<S20>/Selector'
        //   Selector: '<S20>/Selector1'
        //   Sum: '<S20>/Add'
        //   Sum: '<S20>/Add1'
        ...
        rtb_zNAS = ((ABK_P.Trajectorydifference_Value[static_cast<uint16_t>
                     (rtb_Prelookup1_o1 + ABK_P.Bias_Bias_b) + u0_tmp] -
                     rtb_zNAS) * rtb_Prelookup1_o2 + rtb_zNAS) *
          ABK_P.T_refGain_Gain;

        // Saturate: '<S20>/Saturation2' incorporates:
        //   Switch: '<S16>/Switch'

        if (rtb_zNAS > ABK_P.Saturation2_UpperSat) {
          rtb_zNAS = ABK_P.Saturation2_UpperSat;
        } else if (rtb_zNAS < ABK_P.Saturation2_LowerSat) {
          rtb_zNAS = ABK_P.Saturation2_LowerSat;
        }

        // Saturate: '<S17>/Saturation' incorporates:
        //   Switch: '<S16>/Switch'

        if (rtb_MatrixMultiply1 > ABK_P.Saturation_UpperSat_b) {
          rtb_Prelookup1_o2 = ABK_P.Saturation_UpperSat_b;
        } else if (rtb_MatrixMultiply1 < ABK_P.Saturation_LowerSat_f) {
          rtb_Prelookup1_o2 = ABK_P.Saturation_LowerSat_f;
        } else {
          rtb_Prelookup1_o2 = rtb_MatrixMultiply1;
        }
        ...
        rtb_zNAS = (rtb_zNAS + ABK_P.Bias3_Bias) * (rtb_Prelookup1_o2 -
          rtb_MatrixMultiply) + rtb_MatrixMultiply;
      }

      // End of Outputs for SubSystem: '<S16>/Final full opening'

      // SignalConversion generated from: '<S17>/Vector Concatenate' incorporates:
      //   Constant: '<S18>/fullOpen'
      //   Memory: '<S17>/Memory'

      ABK_DW.Memory_PreviousInput[0] = rtb_MatrixMultiply;

      // Update for Memory: '<S17>/Memory3' incorporates:
      //   SignalConversion generated from: '<S17>/Vector Concatenate2'

      ABK_DW.Memory3_PreviousInput[0] = rtb_MatrixMultiply1;
      ABK_DW.Memory3_PreviousInput[1] = rtb_Bypass_vect_idx_1;

      // Update for Memory: '<S16>/Memory'
      ABK_DW.Memory_PreviousInput_g = rtb_OR;
      ...
      ABK_DW.ABKLogsOBSW_BusCreator_BusCreat.FilterCoefficient = 0.0F;
      ABK_DW.ABKLogsOBSW_BusCreator_BusCreat.PrePIDCommand = rtb_Prelookup_o2;
      ABK_DW.ABKLogsOBSW_BusCreator_BusCreat.PostPIDCommand = rtb_MatrixMultiply;
    ...
    ABK_DW.BusAssignment1.BypassActivation = (rtb_OR_l || rtb_Lowercondition);

    // Bias: '<S2>/Bias' incorporates:
    //   UnitDelay: '<S2>/Unit Delay1'

    u0_0 = static_cast<uint8_t>(ABK_DW.UnitDelay1_DSTATE + ABK_P.Bias_Bias_a);

    // Saturate: '<S2>/Saturation'
    if (u0_0 > ABK_P.Saturation_UpperSat_d) {
      // Update for UnitDelay: '<S2>/Unit Delay1'
      ABK_DW.UnitDelay1_DSTATE = ABK_P.Saturation_UpperSat_d;
    } else if (u0_0 < ABK_P.Saturation_LowerSat_c) {
      // Update for UnitDelay: '<S2>/Unit Delay1'
      ABK_DW.UnitDelay1_DSTATE = ABK_P.Saturation_LowerSat_c;
    } else {
      // Update for UnitDelay: '<S2>/Unit Delay1'
    ...
    rtb_zNAS = ABK_P.Constant_Value_c;
  ...
  // End of Outputs for SubSystem: '<Root>/ABK FICA - Autocoding'
...
{
  {
    ...
    // SystemInitialize for Atomic SubSystem: '<Root>/ABK FICA - Autocoding'
    // Start for If: '<S1>/If'
    ...
    // SystemInitialize for Enabled SubSystem: '<S16>/Final full opening'
    // Start for Constant: '<S18>/fullOpen'
    ABK_DW.fullOpen = ABK_P.fullOpen_Value;

    // End of SystemInitialize for SubSystem: '<S16>/Final full opening'
    ...
    // End of SystemInitialize for SubSystem: '<Root>/ABK FICA - Autocoding'
  }
}
