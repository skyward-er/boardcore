...
void NASDAQ0::step()
{
    ...
    if (rtb_AND)
    {
        ...
        else
        {
            ...
            for (k = 0; k < 4; k++)
            {
                ...
                i = k << 2;
                rtb_BackwardSubstitution[i] =
                    NASDAQ0_DW.IdentityMatrix[i + idxout];
                idxout =
            ...
            {
                idxout = i << 2;
                rtb_BackwardSubstitution[idxout + 1] -=
                    rtb_LUFactorization_o1[1] *
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                rtb_BackwardSubstitution[idxout + 3] = rtb_Gain1_b;
                rtb_BackwardSubstitution[idxout + 2] =
                    (rtb_BackwardSubstitution[idxout + 2] -
                     rtb_Gain1_b * rtb_LUFactorization_o1[14]) /
                ...
                rtb_BackwardSubstitution[idxout + 1] =
                    rtb_Gain1_b / rtb_LUFactorization_o1[rtb_Bias1_tmp];
                rtb_Bias1_tmp = 12;
        ...
        for (i = 0; i < 6; i++)
        {
            for (rtb_Bias1_tmp = 0; rtb_Bias1_tmp < 6; rtb_Bias1_tmp++)
            {
                ...
                rtb_Merge_n[rtb_S_tmp_0] =
                    ((rtb_Transpose1[idxout + 1] *
                          rtb_Merge_b[rtb_Bias1_tmp + 6] +
                      rtb_Transpose1[idxout] * rtb_Merge_b[rtb_Bias1_tmp]) +
                     rtb_Transpose1[idxout + 2] *
                         rtb_Merge_b[rtb_Bias1_tmp + 12]) +
                    rtb_Transpose1[idxout + 3] *
    ...
    rtb_AND_e =
        (NASDAQ0_P.Constant_Value_o1 &&
         (NASDAQ0_DW.RateTransition > NASDAQ0_DW.Memory_PreviousInput_a));
    ...
    if (rtb_AND_e)
    {
        ...
        for (idxout = 0; idxout < 6; idxout++)
        {
            ...
            for (rtb_Bias1_tmp = 0; rtb_Bias1_tmp < 6; rtb_Bias1_tmp++)
            {
                ...
                {
                    rtb_Matrix1Norm1 +=
                        rtb_MatrixMultiply[rtb_S_tmp_0 + idxout] *
                        rtb_Merge_l[i_0 + i];
                    rtb_S_tmp_0 += 6;
                }
    ...
    if (rtb_AND_c)
    {
        ...
        for (idxout = 0; idxout < 6; idxout++)
        {
            ...
            rtb_Gain1_b += rtb_Matrix1Norm2_p *
                           static_cast<float>(NASDAQ0_P.HMatrix_Value[idxout]);
            k += 6;
        }
    ...
    for (k = 0; k < 6; k++)
    {
        NASDAQ0_Y.NASDAQLogsOBSW.CovarianceMatrixDiagonal[k] =
            NASDAQ0_DW.Merge[i];
        i += 7;
    }
    ...
    u0 = static_cast<int8_t>(NASDAQ0_DW.UnitDelay2_DSTATE +
